...
export default function BlogSection() {
    return (
        <section className="relative bg-gradient-to-b from-slate-50 via-white to-slate-50 py-28 px-3 sm:px-5 lg:px-7 overflow-hidden">
            {/* Background decorations */}
            <div className="absolute top-0 right-0 w-[520px] h-[520px] bg-blue-100 rounded-full blur-[130px] opacity-40" />
            <div className="absolute bottom-0 left-0 w-[520px] h-[520px] bg-purple-100 rounded-full blur-[130px] opacity-40" />

            <div className="max-w-6xl mx-auto relative z-10">
                {/* Section Header */}
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-5 mb-14">
                    <div>
                        <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-900 text-white mb-5">
                    ...
                    <Link
                        href="/blog"
                        className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-full bg-white border-2 border-slate-900 text-slate-900 font-semibold hover:bg-slate-900 hover:text-white transition-all duration-300 shadow-lg hover:shadow-xl text-sm"
                    >
                        View All Articles
                    ...
                    {featuredPosts.map((post, index) => (
                        ...
                        >
                            <Link href={`/blog/${post.id}`} className="block h-full">
                                <div className={`relative h-full overflow-hidden rounded-2xl ${index === 0 ? 'min-h-[430px]' : 'min-h-[240px]'}`}>
                                    {/* Background gradient */}
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-5 md:p-7">
                                        <div className="flex items-center gap-2.5 text-xs text-white/70 mb-3">
                                            <time>{post.date}</time>
                                        ...
                                        <div className="inline-flex items-center gap-1.5 text-sm text-white font-medium group-hover:gap-3 transition-all duration-300">
                                            Read Article
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
                                            </svg>
