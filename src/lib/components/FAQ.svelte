    ...
    <div class="max-w-6xl mx-auto relative z-10">
        ...
        <div class="text-center mb-16">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto">
                Quick answers to help you understand crypto cards and find the
                support you need.
            </p>
        </div>
            ...
            {#each faqs as faq, index}
                <div
                    class={`group relative overflow-hidden transition-all duration-300 ${
                        openIndex === index
                            ? "bg-[#111] rounded-2xl my-4 border-b border-transparent"
                            : "border-b border-white/10 hover:bg-white/5"
                    }`}
                    style={openIndex === index
                        ? "background-image: repeating-linear-gradient(135deg, #141414 0px, #141414 4px, transparent 4px, transparent 6px);"
                        : ""}
                >
                    <button
                        onclick={() => toggleFAQ(index)}
                        class={`w-full flex items-center justify-between text-left transition-all duration-300 ${openIndex === index ? "p-8 pb-4" : "py-6 px-4"}`}
                    >
                        <span
