<script lang="ts">
    ...
    // Derived values
    let providers = $derived(Array.from(new Set(cryptoCards.map(card => card.provider))));
    let regions = $derived(Array.from(new Set(cryptoCards.flatMap(card => card.region))).sort());

    let filteredCards = $derived(cryptoCards.filter(card => {
    ...
    let sortedCards = $derived([...filteredCards].sort((a, b) => {
...
<main class="min-h-screen bg-white">
    <!-- Hero Section -->
    <div class="bg-gradient-to-b from-slate-50 to-white pt-28 pb-16 px-3 sm:px-5 lg:px-7 border-b border-gray-200">
        ...
        <div class="flex flex-col lg:flex-row gap-8 relative">
            ...
            `}>
                <div class="p-5 lg:p-0 lg:sticky lg:top-24 space-y-8">
                    ...
                    <!-- Network Filter -->
                    <div class="space-y-3">
                        <h3 class="text-sm font-bold text-slate-900 uppercase tracking-wider">Network</h3>
                        <div class="space-y-2">
                            {#each ["all", "Visa", "Mastercard"] as network}
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <div class={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${selectedNetwork === network ? "border-blue-600 bg-blue-600" : "border-slate-300 group-hover:border-slate-400 bg-white"}`}>
                                    ...
                                    />
                                    <span class={`text-sm ${selectedNetwork === network ? "text-slate-900 font-medium" : "text-slate-600 group-hover:text-slate-900"}`}>
                    ...
                    <!-- Region Filter -->
                    <div class="space-y-3">
                        <h3 class="text-sm font-bold text-slate-900 uppercase tracking-wider">Region</h3>
                        <div class="relative">
                            <select
                    ...
                    <!-- Material Filter -->
                    <div class="space-y-3">
                        <h3 class="text-sm font-bold text-slate-900 uppercase tracking-wider">Card Material</h3>
                        <div class="flex flex-wrap gap-2">
                            {#each ["all", "metal", "plastic"] as type}
                                <button
                                    onclick={() => selectedType = type as FilterType}
                    ...
                    <!-- Additional filters would go here (Staking, Cashback, Fees, Features, Provider) - keeping it concise for now -->
                     ...
                     <!-- Provider Filter -->
                      <div class="space-y-3">
                        <h3 class="text-sm font-bold text-slate-900 uppercase tracking-wider">Provider</h3>
                        <div class="space-y-2">
                             <label class="flex items-center gap-3 cursor-pointer group">
                                <div class={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${selectedProvider === "all" ? "border-blue-600 bg-blue-600" : "border-slate-300 group-hover:border-slate-400 bg-white"}`}>
                                    {#if selectedProvider === "all"}
                                        <div class="w-2 h-2 rounded-full bg-white"></div>
                                    {/if}
                                </div>
                                <input type="radio" name="provider" class="hidden" value="all" bind:group={selectedProvider}/>
                                <span class={`text-sm ${selectedProvider === "all" ? "text-slate-900 font-medium" : "text-slate-600 group-hover:text-slate-900"}`}>
                                    All Providers
                                </span>
                            </label>
                            {#each providers as provider}
                                <label class="flex items-center gap-3 cursor-pointer group">
                                    <div class={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${selectedProvider === provider ? "border-blue-600 bg-blue-600" : "border-slate-300 group-hover:border-slate-400 bg-white"}`}>
                                        {#if selectedProvider === provider}
                                            <div class="w-2 h-2 rounded-full bg-white"></div>
                                        {/if}
                                    </div>
                                    <input type="radio" name="provider" class="hidden" value={provider} bind:group={selectedProvider}/>
                                    <span class={`text-sm ${selectedProvider === provider ? "text-slate-900 font-medium" : "text-slate-600 group-hover:text-slate-900"}`}>
                                        {provider}
                                    </span>
                                </label>
                            {/each}
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-sm font-medium text-slate-700 whitespace-nowrap">Sort by:</span>
                        <div class="relative">
                            <select
                                bind:value={sortBy}
                ...
                <!-- Quick Filters (Top) -->
                <div class="flex flex-wrap items-center gap-6 mb-6 pb-6 border-b border-slate-100">
                     <!-- Network Quick Filter -->
                    <div class="flex items-center gap-3">
                        <span class="text-sm font-bold text-slate-900 uppercase tracking-wider">Network:</span>
                        <div class="flex gap-2">
                            {#each ["all", "Visa", "Mastercard"] as network}
                                <button
                                    onclick={() => selectedNetwork = network as FilterNetwork}
                                    class={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${selectedNetwork === network
                    ...
                    <!-- Type Quick Filter -->
                    <div class="flex items-center gap-3">
                        <span class="text-sm font-bold text-slate-900 uppercase tracking-wider">Type:</span>
                        <div class="flex gap-2">
                            {#each ["all", "metal", "plastic"] as type}
                                <button
                                    onclick={() => selectedType = type as FilterType}
                                    class={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${selectedType === type
                                        ? "bg-slate-900 border-slate-900 text-white"
                                        : "bg-white border-slate-200 text-slate-600 hover:border-slate-300 hover:bg-slate-50"
                                        }`}
                                >
                ...
                <div class="flex flex-wrap gap-2 mb-6">
                    {#if selectedNetwork !== "all"}
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 text-slate-700 text-xs font-medium rounded-full border border-slate-200">
                            Network: {selectedNetwork}
                            <button onclick={() => selectedNetwork = "all"} class="hover:text-slate-900"><svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" /></svg></button>
                        </span>
                    {/if}
                        ...
                        >
                            ...
                            <div class="relative h-48 bg-slate-100 overflow-hidden">
                                ...
                                <div class="absolute inset-4 flex items-center justify-center">
                                    <div class={`relative w-full max-w-[280px] h-[160px] bg-gradient-to-br ${card.color} rounded-xl shadow-xl transform group-hover:scale-105 group-hover:-rotate-1 transition-all duration-500`}>
                                        ...
                                        <div class="absolute bottom-4 right-4 flex gap-2">
                                            <span class="px-2 py-0.5 bg-black/40 backdrop-blur-sm text-white text-[10px] font-medium rounded uppercase">{card.network}</span>
                                        </div>

                                        <!-- Verification Status Badge -->
                                        <div class="absolute top-4 right-4">
                                            <span class={`px-2 py-1 text-[10px] font-semibold rounded ${card.verificationStatus === "verified"
                            ...
                            <div class="p-5">
                                <div class="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 class="text-lg font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{card.name}</h3>
                                        <p class="text-sm text-slate-500">{card.provider}</p>
                                    </div>
                                    <div class="flex flex-col items-end">
                                        <div class="flex items-center gap-1 bg-amber-50 px-2 py-1 rounded-lg">
                                            <span class="text-amber-500 text-xs">★</span>
                                            <span class="text-sm font-bold text-slate-900">{card.rating}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4 mb-4">
                                    <div class="bg-slate-50 p-3 rounded-xl">
                                        <p class="text-[10px] uppercase tracking-wider text-slate-500 font-semibold mb-1">Cashback</p>
                                        <p class="text-sm font-bold text-slate-900">{card.baseCashback} - {card.maxCashback}</p>
                                    </div>
                                    <div class="bg-slate-50 p-3 rounded-xl">
                                        <p class="text-[10px] uppercase tracking-wider text-slate-500 font-semibold mb-1">Annual Fee</p>
                                        <p class="text-sm font-bold text-slate-900">{card.annualFee}</p>
                                    </div>
                                </div>

                                <div class="flex gap-2 mb-5 overflow-x-auto pb-2 scrollbar-none">
                                    {#each card.features.slice(0, 3) as feature}
                                        <span class="whitespace-nowrap px-2.5 py-1 bg-slate-100 text-slate-600 text-[10px] font-medium rounded-full border border-slate-200">
                                            {feature}
                                        </span>
                                    {/each}
                                </div>
                                ...
                                <div class="flex gap-3">
                                    <button class="flex-1 py-2.5 bg-slate-900 text-white text-sm font-bold rounded-xl hover:bg-blue-600 transition-all shadow-lg hover:shadow-blue-500/30">
                                        Apply Now
                                    </button>
                                </div>
                            </div>
